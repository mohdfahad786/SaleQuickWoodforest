...
//live events
	$(document)
		...
		.on('click','.new-item-adder button',function(e){
			...
			newElem.find('.item_row:last-child').append('<span class="remove-invoice-item"> <img src="https://salequick.com/new_assets/img/delete.png" alt="del"> </span>');
	...
	//pos data table events
		...
		.on('click','.custom-stepper-form .third-step .next-step',function(e){
			...
			if(signUpStepThird($wrapper)) {
				...
				// console.log()
				//go to fourth step
				if($('.custom-stepper-form .third-step .first_busi_owner_section').hasClass('pointer')) {
		            alert('Save Business Owner Info first.');
		            return false;
					...
					var ownerArr = [];
					$('.custom-stepper-form .third-step .busi_owner_row input.rowHiddenInput').each(function(){
						// console.log($(this));
						var arr1 = $(this).data();
						ownerArr.push(arr1);
					})
