...
//convert sale summery json to html table
  function saleSummeryPdfTableConvertor($wraper,jd){
    var newTable=$('<table><thead><th>Amount</th><th>Tax</th><th>Type</th><th>Date</th><th>Referece</th><th>Name</th><th>Items</th></thead><tbody></tbody></table>');
    var allRow='';
    console.log(jd)
    console.log(typeof jd)
    if(!jd){
      allRow='<tr><td colspan="7" align="center">No data</td></tr>';
    }
    else
    {
      if(typeof jd != 'object')
      ...
      jsonData.forEach(function(val, i){
        ...
        {
         allRow='<tr><td>'+val.Amount+'</td><td>'+val.Tax+'</td><td>'+val.Type+'</td><td>'+val.Date+'</td><td>'+val.Referece+'</td><td>'+val.Name+'</td><td>'+val.Items+'</td></tr>';    
        }
      });
    }
  ...
  jQuery(function($) {
    ...
    //----datatables --------
     ...
     var dtConfigOrderSort2={
      ...
      $('#transaction_recurring_dt').DataTable(dtTransactionsConfig);
...
//live events
  $(document)
    ...
    .on('click','.new-item-adder button',function(e){
      ...
      newElem.find('.col:last-child .form-group').append('<span class="remove-invoice-item"> <img src="../new_assets/img/delete.png" alt="del"> </span>');
  ...
  //pos data table events
    ...
    .on('click','.custom-stepper-form .second-step .next-step',function(e){
      ...
      if(signUpStepSecond($wrapper))
      {
        ...
        var mSignupStepS={};
            mSignupStepS.mlbName=$wrapper.find('.form-control[name="mlbName"]').val();
            mSignupStepS.mdba=$wrapper.find('.form-control[name="mdba"]').val();
            mSignupStepS.mwebsite=$wrapper.find('.form-control[name="mwebsite"]').val();
            mSignupStepS.mepvolume=$wrapper.find('.form-control[name="mepvolume"]').val();
            mSignupStepS.meidno=$wrapper.find('.form-control[name="meidno"]').val();
            mSignupStepS.mbaddress=$wrapper.find('.form-control[name="mbaddress"]').val();
            $(this).find('span').addClass('fa fa-spinner fa-spin');
    ...
    .on('click','.custom-stepper-form .third-step .next-step',function(e){
      ...
      if(signUpStepThird($wrapper))
      {
        ...
        console.log('false-run');
        var mSignupStepTh={};
        mSignupStepTh.mfname=$wrapper.find('.form-control[name="mfname"]').val();
        mSignupStepTh.mdob=$wrapper.find('.form-control[name="mdob"]').val();
        mSignupStepTh.mopercentage=$wrapper.find('.form-control[name="mopercentage"]').val();
        mSignupStepTh.mssno=$wrapper.find('.form-control[name="mssno"]').data('val');
        mSignupStepTh.mdlno=$wrapper.find('.form-control[name="mdlno"]').data('val');
        mSignupStepTh.mhaddress=$wrapper.find('.form-control[name="mhaddress"]').val();
        $(this).find('span').addClass('fa fa-spinner fa-spin');
    ...
    .on('click','.custom-stepper-form .fourth-step .submit-step',function(e){
      ...
      {
        //call ajax &  submit step first
        var $mWrapper=$(this).closest('.steps-wrapper');
        var mSignupStepFth={};
            ...
            mSignupStepFth.mrouteno=$mWrapper.find('.form-control[name="mrouteno"]').val();
            mSignupStepFth.maccno=$mWrapper.find('.form-control[name="maccno"]').val();
            mSignupStepFth.mconfaccno=$mWrapper.find('.form-control[name="mconfaccno"]').val();
            mSignupStepFth.mbankname=$mWrapper.find('.form-control[name="mbankname"]').val();
