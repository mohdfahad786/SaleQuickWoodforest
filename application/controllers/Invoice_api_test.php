...
class Invoice_api_test extends REST_Controller {
...
// start 
		...
		//end
				...
				// 		}
						...
						$status = parent::HTTP_OK;
						if(!empty($id)){
						     $response = ['status' => $status, 'successMsg' => 'Payment Request Send Successfull','id' => $id];
...
public function refund_invoice_detail_post()
{
    ...
    $merchant_id = $_POST['merchant_id'];
	        	...
	        	$stmt1 = $this->db->query("SELECT logo,business_name,business_dba_name,date_c,business_number FROM merchant WHERE id ='".$merchant_id."' ");
	...
	$stmt = $this->db->query("SELECT invoice_no,ip_a,transaction_id, card_type,card_no,reference,amount FROM customer_payment_request WHERE id ='".$p_id."' and status = '".$status."'");
