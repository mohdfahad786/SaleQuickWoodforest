...
class Merchant_document extends CI_Controller {
	...
	public function add_ongoing_note() {
		$data['meta'] = 'Add Ongoing Note';
		...
		if($_POST) {
			...
			$note_sr_id=$this->input->post('srNo') ? $this->input->post('srNo') : '';
			// $myfile=$_FILES['attachment']['name'];
			$error_msg = '';
			if($_FILES['attachment']['name'][0] != '') {
				foreach($_FILES['attachment']['name'] as $key => $val) {
					$attach_arr = explode('.', $_FILES['attachment']['name'][$key]);
					$imageExtention = pathinfo($_FILES["attachment"]["name"][$key], PATHINFO_EXTENSION);
	                $new_name = $attach_arr[0].date().time().'.'.$imageExtention;
					// echo $new_name;die;
            ...
            if($myfile2!="") {
				...
				$imageExtention2 = pathinfo($_FILES["attachment2"]["name"], PATHINFO_EXTENSION);
                $new_name2 = $attach_arr2[0].date().time().'.'.$imageExtention2;
				// echo $new_name2;die;
		...
		$srNo=random_int(100000, 999999);
		$data['srNo']=$srNo;
		$this->load->view('admin/add_ongoing_note', $data);
	}
	public function add_sub_ongoing_note() {
		// print_r($_POST);die;
		$data['meta'] = 'Add Sub Ongoing Note';
		$merchant_id = $this->uri->segment(3);
		$data['merchant_id'] = $merchant_id;
		$creator_id = $this->uri->segment(4);
		$data['creator_id'] = $creator_id;
		$note_sr_id = $this->uri->segment(5);
		$data['note_sr_id'] = $note_sr_id;
		// $sub_note_id = $this->uri->segment(6);
		// $data['sub_note_id'] = $sub_note_id;
		$data['subject'] = $this->db->select('subject')->where('id', $creator_id)->get('merchant_ongoing_notes')->row();
		$merchant = $this->db->select('business_dba_name,email')->where('id', $merchant_id)->get('merchant')->row();
		$data['merchant'] = $merchant;
		...
		if($_POST) {
			...
			$note = $this->input->post('note') ? $this->input->post('note') : '';
			$created_by = $this->session->userdata('id');
			$sub_note_id=$this->input->post('sub_note_id') ? $this->input->post('sub_note_id') : '';
			// $myfile=$_FILES['attachment']['name'];
			$error_msg = '';
			// if($_FILES['attachment']['name'][0] != '') {
			// 	foreach($_FILES['attachment']['name'] as $key => $val) {
	  ...
	  //               } else {
	  //               	$this->session->set_flashdata('error',$this->upload->display_errors());
			//             redirect(base_url('merchant_document/ongoing_sub_note/'.$merchant_id.'/'.$creator_id.'/'.$note_sr_id));
	  //               }
			// 	}
   //          }
   ...
   //          $attachment2=$uploadedFileName2;
			...
			$ins_data = array(
				'merchant_id' => $merchant_id,
				'sub_note_creatorId' => $creator_id,
				'sub_note' => $note,
				'subNote_sr_id'=>$sub_note_id,
				'created_by' => $created_by,
				
			);
			// echo '<pre>';print_r($ins_data);die;
			$this->db->insert('sub_ongoing_note', $ins_data);
			// echo $this->db->last_query();die;
			$this->session->set_flashdata('success','Sub note added successfully.');
            // redirect(base_url('merchant_document/ongoing_sub_note/'.$merchant_id.'/'.$creator_id.'/'.$note_sr_id));
            redirect(base_url('merchant_document/ongoing_notes/'.$merchant_id));

		}
		$sub_note_id=random_int(10000000, 99999999);
		$data['sub_note_id']=$sub_note_id;
		// echo '<pre>';print_r($data);die;
		$this->load->view('admin/add_sub_ongoing_note', $data);
	}
	public function ongoing_notes_original() {
		...
		if($_POST) {
			// $imageExtention = pathinfo($_FILES["attachment"]["name"], PATHINFO_EXTENSION);
			// echo $imageExtention;die;
			$merchant_id = $this->input->post('merchant_id') ? $this->input->post('merchant_id') : '';
			$subject = $this->input->post('subject') ? $this->input->post('subject') : '';
			$note = $this->input->post('note') ? $this->input->post('note') : '';
			$created_by = $this->session->userdata('id');
	...
	public function remove_pdf_file($id,$onId) {
		$responseArr = array();
		
		// echo $this->db->last_query();die;
		if($this->db->query("DELETE from ongoing_notes_attachment where id=".$id)) {
			$responseArr['status'] = 200;
			
        	$responseArr['message'] = 'PDF removed successfully.';
        	$responseArr['post'] = 'PDF removed successfully.';

		} else {
			$responseArr['status'] = 501;
			$responseArr['message'] = 'Something went wrong. Please try again.';
		}
		// $this->session->set_flashdata('success','Pdf file removed successfully.');
        // redirect(base_url('admin/add_ongoing_notes/'.$merchant_id));
		echo json_encode($responseArr);die;
		//$this->db->where("id",$id);
		//$this->db->delete("ongoing_notes_attachment");
		//echo $this->db->last_query();die;

		//$this->db->select('merchant_id');
		//$query=$this->db->get('ongoing_notes_attachment')->result_array();
		//$merchant_id=$query[0]['merchant_id'];
		$this->session->set_flashdata('success', 'Pdf file removed successfully.');
        //redirect(base_url('merchant_document/edit_ongoing_notes/'.$merchant_id.'/'.$onId));
	}
	...
	public function get_pdf($id,$merchant_id) {
		//echo 'hi';die;
		$responseArr = array();
		$query1 = $this->db->query("SELECT note_sr_id,attachment,attachment2 from merchant_ongoing_notes where id=".$id." and merchant_id=".$merchant_id)->row();
		$note_sr_id = $query1->note_sr_id;
		$query2=$this->db->query("SELECT attachment from ongoing_notes_attachment where note_id=".$note_sr_id)->result_array();
		// echo $this->db->last_query();die;
		// echo '<pre>';print_r($query1);die;
		if($query2!='') {
			$responseArr['status'] = 200;
			// echo $this->db->last_query();die;
			$responseArr['pdf'] = json_encode($query2);
			$responseArr['pdf2']['attachment1'] = $query1->attachment;
			$responseArr['pdf2']['attachment2'] = $query1->attachment2;
        	$responseArr['message'] = 'PDF found';
        	$responseArr['post'] = 'PDF found.';
	...
	public function ongoing_sub_note_popup() {
		// print_r($_POST);die;
		// $data['meta'] = 'Ongoing Sub Notes';
		...
		$ongoing_notes = $this->db->query("SELECT s.id,s.created_at,subject,sub_note,(SELECT name from admin WHERE id=s.created_by) as 'created_by' from sub_ongoing_note s,merchant_ongoing_notes m WHERE s.merchant_id='".$merchant_id."' and s.sub_note_creatorId='".$creator_id."' and m.id='".$creator_id."' order by s.id desc;")->result();
		// $ongoing_notes=$ongoing_notes[0];
			// echo $this->db->last_query();die;
		// echo '<pre>';print_r($ongoing_notes);die;
		// $ongoing_notes=json_encode($ongoing_notes);
		
		// $this->session->set_flashdata('success','Pdf file removed successfully.');
        // redirect(base_url('admin/add_ongoing_notes/'.$merchant_id));
			echo json_encode($ongoing_notes);die;
		
	}
	public function get_pdf_subnotes($sid,$mid)
	{
		//echo 'hi';die;
		$responseArr = array();
		$query=$this->db->query("SELECT o.attachment from ongoing_notes_attachment o join sub_ongoing_note s on o.sub_note_id=s.subNote_sr_id join merchant_ongoing_notes m on m.id= ".$mid." and s.sub_note_creatorId=".$mid." and s.id=".$sid)->result_array();

		// echo $this->db->last_query();die;
		if($query!='') {
				$responseArr['status'] = 200;
				// echo $this->db->last_query();die;
				$responseArr['pdf']=json_encode($query);
	        	$responseArr['message'] = 'PDF found.';
	        	$responseArr['post'] = 'PDF found.';
	...
	public function get_pdf_subnotes_new($mid)
	{
		//echo 'hi';die;
		$responseArr = array();
		$data=array();
		$ids=$this->db->query("select id from sub_ongoing_note where sub_note_creatorId=".$mid." order by id desc")->result_array();
		// echo $this->db->last_query();die;
		for ($i=0; $i <sizeof($ids) ; $i++) { 
			$sid= $ids[$i]['id'];
			$query=$this->db->query("SELECT o.attachment from ongoing_notes_attachment o join sub_ongoing_note s on o.sub_note_id=s.subNote_sr_id join merchant_ongoing_notes m on m.id= ".$mid." and s.sub_note_creatorId=".$mid." and s.id=".$sid)->result_array();
			$data[$i].=json_encode($query);
	...
	public function ongoing_sub_note() {
		// print_r($_POST);die;
		$data['meta'] = 'Ongoing Sub Notes';
		...
		$ongoing_notes = $this->db->query("SELECT s.id,s.created_at,subject,sub_note,(SELECT name from admin WHERE id=s.created_by) as 'created_by' from sub_ongoing_note s,merchant_ongoing_notes m WHERE s.merchant_id='".$merchant_id."' and s.sub_note_creatorId='".$creator_id."' and m.id='".$creator_id."' order by s.id desc;")->result_array();
			// echo $this->db->last_query();die;
		$data['ongoing_notes'] = $ongoing_notes;
		// echo '<pre>';print_r($data);die;
		$this->load->view('admin/ongoing_sub_note', $data);
	}
	...
	public function sorted_ongoing_notes() {
		...
		if(!empty($ongoing_notes)) {
			foreach ($ongoing_notes as $key => $val) {
				...
				$sub_note_link = base_url('merchant_document/ongoing_sub_note/'.$merchant_id.'/'.$val['id'].'/'.$val['note_sr_id']);
	                        ...
	                        <div style="display: flex !important;">
	                            ...
	                            <div class="dropdown dt-vw-del-dpdwn text-right">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(45px, 31px, 0px);">
                                        <a href="'.$edit_link.'" class="dropdown-item"><i class="fa fa-pencil"></i>Edit</a>
                                        <a class="dropdown-item" href="#" onClick="delNote('.$merchant_id.','.$val["id"].')"><i class="fa fa-trash"></i> Delete</a>
                                        <a href="javascript:void(0)" class="pdfModal dropdown-item" data-id="'.$val["id"].'"><i class="fa fa-paperclip"></i>Attachment</a>
                                        <a href="'.$sub_note_link.'" class="dropdown-item"><i class="fa fa-sticky-note-o"></i> Sub Note</a>
                                    </div>
                                </div>'.$attachment_data.'
	                        </div>
	...
	public function sorted_ongoing_sub_notes() {
		$response = array();
		// $merchant_id = $this->input->post('merchant_id') ? $this->input->post('merchant_id') : '';
		$value = $this->input->post('value') ? $this->input->post('value') : '';
		...
		echo $merchant_id.','.$creator_id.','.$note_sr_id;die;
	...
	public function get_single_ongoing_note() {
		...
		// echo $note->updated_by;die;
		if(!empty($note->updated_by)) {
			$arr2 = $this->db->select('name as updated_by')->where('id',$note->updated_by)->from('admin')->get()->row();
			// print_r($arr2);die;
			$note->updated_by = $arr2->updated_by;
		}
	...
	public function get_single_ongoing_sub_note() {
		$id = $_POST['id'];
		$note = $this->db->get_where('sub_ongoing_note', ['id' => $id])->row();
	...
	public function delete_ongoing_notes(){
		$merchant_id = $this->uri->segment(3);
		$id = $this->uri->segment(4);
		$data['on_id'] = $id;
		
		$this->db->query("DELETE from merchant_ongoing_notes where id=".$id." and merchant_id=".$merchant_id);
		$data['meta'] = 'Ongoing Notes';
		...
		$ongoing_notes = $this->db->select('on.*, a.name as created_by')
			...
			->get()->result_array();
		$data['ongoing_notes'] = $ongoing_notes;
		$response = ['status' => '200', 'successMsg' => 'Note deleted'];

		//$this->load->view('admin/ongoing_notes', $data);
		echo json_encode($response);die;
	}
	public function delete_sub_ongoing_notes(){
		$merchant_id = $this->uri->segment(3);
		$id = $this->uri->segment(4);
		$data['on_id'] = $id;
		$creator_id=$this->uri->segment(5);
		$data['creator_id']=$creator_id;
		$note_sr_id=$this->uri->segment(6);
		$data['note_sr_id']=$note_sr_id;
		// echo $creator_id.'/'.$note_sr_id;die;
		$query=$this->db->query("SELECT subNote_sr_id from sub_ongoing_note where id=".$id)->result_array();
		$sub_note_id=$query[0]['subNote_sr_id'];
		
		$this->db->query("DELETE from sub_ongoing_note where id=".$id." and merchant_id=".$merchant_id);
		$this->db->query("DELETE from ongoing_notes_attachment where sub_note_id=".$sub_note_id." and merchant_id=".$merchant_id);
		$data['meta'] = 'Ongoing Sub Notes';
...
//		$data['merchant'] = $merchant;
		$response = ['status' => '200', 'successMsg' => 'Sub Note deleted successfully'];
		//redirect(base_url('merchant_document/ongoing_sub_note/'.$merchant_id.'/'.$creator_id.'/'.$note_sr_id));
		echo json_encode($response);die;
	}
	public function edit_ongoing_notes() {
		...
		$data['ongoing_notes'] = $ongoing_notes;
		//echo '<pre>';print_r($ongoing_notes->note_sr_id);die;

		$attachedFiles=$this->db->query("SELECT id,attachment from ongoing_notes_attachment where merchant_id=".$merchant_id." and note_id=".$ongoing_notes->note_sr_id)->result_array();
		$data['attachedFiles'] = $attachedFiles;
		//echo $this->db->last_query();die;
		//  echo '<pre>';print_r($attachedFiles);die;
	...
	public function edit_sub_ongoing_note() {
		// echo $this->uri->segment(4);die;
		$data['meta'] = 'Edit Sub Ongoing Notes';

		$merchant_id = $this->uri->segment(3);
		$data['merchant_id'] = $merchant_id;

		$mid = $this->uri->segment(4); //merchant ongoing notes id
		$data['mid']= $mid;
		$sid = $this->uri->segment(5);
		$data['on_id'] = $sid; //subongoing notes id
		$note_sr_id = $this->uri->segment(6);
		$data['note_sr_id'] = $note_sr_id; //subongoing notes id
		 
		// echo 'mid'.$mid.'<br>';
		$merchant = $this->db->select('business_dba_name,email')->where('id', $merchant_id)->get('merchant')->row();
		$data['merchant'] = $merchant;

		$ongoing_notes = $this->db->select('subject')->where('id', $mid)->get('merchant_ongoing_notes')->row();
		$data['ongoing_notes'] = $ongoing_notes;
		$sub_note=$this->db->select('sub_note')->where('id',$sid)->get('sub_ongoing_note')->row();
		$data['sub_note']=$sub_note;
		// echo '<pre>';print_r($data);die;

		$attachedFiles=$this->db->query("SELECT s.id,o.id as 'pdfid',o.attachment from ongoing_notes_attachment o join sub_ongoing_note s on o.sub_note_id=s.subNote_sr_id join merchant_ongoing_notes m on m.id= ".$mid." and s.sub_note_creatorId=".$mid." and s.id=".$sid)->result_array();
		$data['attachedFiles'] = $attachedFiles;

		
		$sub_note_id=$this->db->select('subNote_sr_id')->where('id',$sid)->get('sub_ongoing_note')->row();
		$data['sub_note_id']=$sub_note_id;
		//echo $this->db->last_query();die;
		// echo '<pre>';print_r($data);die;
		...
		if($_POST) {
			...
			$this->db->where('id', $on_id);
			$this->db->update('sub_ongoing_note', $up_data);
			// echo $this->db->last_query();die;
			// $this->db->insert('merchant_ongoing_notes', $ins_data);
			$this->session->set_flashdata('success','Sub Ongoing notes updated successfully.');
            // redirect(base_url('admin/ongoing_sub_note/'.$merchant_id.43));
            // echo $mid;die;
            redirect(base_url('merchant_document/ongoing_notes/'.$merchant_id));
		}
		$this->load->view('admin/edit_sub_ongoing_note', $data);
	}
	...
	public function all_pdfs() {
		...
		foreach ($all_pdfs as $key => $pdf) {
			...
			$package['pdf_label'] = !empty($pdf['pdf_label']) ? $pdf['pdf_label'] : 'NA';
		...
		foreach ($agreement_mails as $key => $mail) {
			if(!empty($mail['ip_address'])) {
				...
				$file_name1 = 'Sub_Merchant_Agreement_'.$file_name.date('Ymdhis', strtotime($mail['add_datetime'])).'.pdf';
				$file_name2 = 'Privacy_Policy_'.$file_name.date('Ymdhis', strtotime($mail['add_datetime'])).'.pdf';
				$file_name3 = 'Terms_and_Conditions_'.$file_name.date('Ymdhis', strtotime($mail['add_datetime'])).'.pdf';
				// echo $file_name;die;
				...
				$package['pdf_label'] = 'Merchant Agreement';
				$package['file_name'] = $file_name;
				$package['file_name1'] = $file_name1;
				$package['file_name2'] = $file_name2;
				$package['file_name3'] = $file_name3;
	...
	public function upload_pdf_file_old() {
	...
	public function all_upload_pdf_file() {
		$responseArr = array();
		// echo '<pre>';print_r($_FILES);die;
		//$pst=$_POST['variable']
    	...
    	if($file_pdf != "") {
      		...
      		if (!$this->upload->do_upload('file')) {
        		...
        		$responseArr['row_id'] = '';
        		$responseArr['file_name'] =$file_name_new;
        		$responseArr['message'] = 'Error! '.$file_name_new .'=> '.$this->upload->display_errors();
				...
				if($this->db->affected_rows() != 1) {
					...
					$responseArr['status'] = 200;
					$responseArr['row_id'] = $insert_id;
	        		$responseArr['message'] = 'PDF uploaded successfully.';
	        		$responseArr['post'] = 'PDF uploaded successfully.';
				}
      		}
    		...
    		$responseArr['status'] = 501;
    		$responseArr['row_id'] = '';
    		$responseArr['message'] = 'No pdf file selected.';
    	}
    	echo json_encode($responseArr);die;
	}
	...
	public function upload_pdf_file() {
    	...
    	if($file_pdf != "") {
      		...
      		if (!$this->upload->do_upload('file')) {
				...
				$this->db->insert('ongoing_notes_attachment', $ins_data);
				$insert_id = $this->db->insert_id();
				// echo $this->db->last_query();die;
				if($this->db->affected_rows() != 1) {
					$responseArr['status'] = 501;
	...
	public function upload_pdf_file_of_edit() {
		$responseArr = array();
		 //print_r($_POST['srNo']);die;
		//$pst=$_POST['variable']
		$file_pdf = $_FILES['file']['name'];
		$file_name = str_replace(' ','_',$_FILES['file']['name']);
		$file_name_new = time() . '_' . $file_name;
	...
	public function upload_sub_pdf_file() {
		$responseArr = array();
		 //print_r($_POST['srNo']);die;
		echo 'hi';
		 // print_r($_POST);die;
	...
	public function update_pdf_label() {
		...
		if($this->db->update('merchant_document', $up_data)) {
			echo 200;die;
		} else {
			echo 500;die;
		}
	...
	public function delete_pdf() {
		...
		// echo $pdf_id;die;
		$this->db->where('id', $pdf_id);
		if($this->db->delete('merchant_document')) {
			echo 200;die;
		} else {
			echo 500;die;
		}
	...
	public function sub_merchant_agreement() {
		$data = array();
		...
		$data['agreement_mail'] = $agreement_mail;
		$data['merchant'] = $merchant;

		$file_name = str_replace(" ","_",$merchant->business_dba_name);
		$file_name = str_replace("'","",$file_name);
		$file_name = 'Sub_Merchant_Agreement_'.$file_name.date('Ymdhis', strtotime($agreement_mail->add_datetime)).'.pdf';

		$pdf_html = '<h1 style="text-align: center;">SUB MERCHANT AGREEMENT</h1>
		...
		<table style="width:100%;border-collapse: collapse;">
		    <tbody>
		        <tr>
		            <th style="text-align: left;border: 1px solid #000;width: 40%;">Processing Fees</th>
		            <th style="width:60%;text-align:right;text-align: left;border: 1px solid #000;"></th>
		        </tr>
		        <tr>
		            <td style="width: 40%;border: 1px solid #000;">Card Present</td>
		            <td style="width:60%;text-align:right;border: 1px solid #000;">2.65%</td>
		        </tr>
		        <tr>
		            <td style="width: 40%;border: 1px solid #000;">Card Not Present</td>
		            <td style="width:60%;text-align:right;border: 1px solid #000;">2.90%</td>
		        </tr>
		        <tr>
		            <th style="width: 40%;text-align: left;border: 1px solid #000;">Authorization Fee</th>
		            <th style="width:60%;text-align:right;text-align: left;border: 1px solid #000;"></th>
		        </tr>
		        <tr>
		            <td style="width: 40%;border: 1px solid #000;">Card Present</td>
		            <td style="width:60%;text-align:right;border: 1px solid #000;">$0.10</td>
		        </tr>
		        <tr>
		            <td style="width: 40%;border: 1px solid #000;">Card Not Present</td>
		            <td style="width:60%;text-align:right;border: 1px solid #000;">$0.30</td>
		        </tr>
		        <tr>
		            <th style="width: 40%;text-align: left;border: 1px solid #000;">Chargeback Fee (per incident)</th>
		            <td style="width:60%;text-align:right;border: 1px solid #000;">$25.00</td>
		        </tr>
		        <tr>
		            <th style="width: 40%;text-align: left;border: 1px solid #000;">Annual Maintenance Fee</th>
		            <td style="width:60%;text-align:right;border: 1px solid #000;">$100.00</td>
		        </tr>
		        <tr>
		            <th style="width: 40%;text-align: left;border: 1px solid #000;">PCI Non-Compliance (per month)</th>
		            <td style="width:60%;text-align:right;border: 1px solid #000;">$25.00</td>
		        </tr>
		        <tr>
		            <th style="width: 40%;text-align: left;border: 1px solid #000;">Gateway Access (per month)</th>
		            <td style="width:60%;text-align:right;border: 1px solid #000;">$15.00</td>
		        </tr>
		    </tbody>
		</table>
		...
		$title = "Sub Merchant Agreement - ".$merchant->business_dba_name;
        $title_head = '';
	...
	public function privacy_policy() {
		...
		$file_name = str_replace(" ","_",$merchant->business_dba_name);
		$file_name = str_replace("'","",$file_name);
		$file_name = 'Privacy_Policy_'.$file_name.date('Ymdhis', strtotime($agreement_mail->add_datetime)).'.pdf';

		$pdf_html = '<h1 style="text-align: center;">PRIVACY POLICY</h1>
		...
		$obj_pdf = new TCPDF('P', PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		$obj_pdf->SetCreator(PDF_CREATOR);
		$title = "Privacy Policy - ".$merchant->business_dba_name;
        $title_head = '';
		$obj_pdf->SetTitle($title);
		$obj_pdf->setHeaderFont(Array(PDF_FONT_NAME_MAIN, '', PDF_FONT_SIZE_MAIN));
		$obj_pdf->setFooterFont(Array(PDF_FONT_NAME_DATA, '', PDF_FONT_SIZE_DATA));
	...
	public function terms_and_conditions() {
		...
		$file_name = str_replace(" ","_",$merchant->business_dba_name);
		$file_name = str_replace("'","",$file_name);
		$file_name = 'Terms_and_Conditions_'.$file_name.date('Ymdhis', strtotime($agreement_mail->add_datetime)).'.pdf';

		$pdf_html = '<h1 style="text-align: center;">TERMS AND CONDITIONS</h1>
		...
		$obj_pdf = new TCPDF('P', PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
		$obj_pdf->SetCreator(PDF_CREATOR);
		$title = "Terms and Conditions - ".$merchant->business_dba_name;
        $title_head = '';
		$obj_pdf->SetTitle($title);
		$obj_pdf->setHeaderFont(Array(PDF_FONT_NAME_MAIN, '', PDF_FONT_SIZE_MAIN));
		$obj_pdf->setFooterFont(Array(PDF_FONT_NAME_DATA, '', PDF_FONT_SIZE_DATA));
