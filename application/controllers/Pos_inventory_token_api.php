...
define('UPLOAD_POS_PATH', 'https://salequick.com/uploads/item_image/');
 ...
 class Pos_inventory_token_api extends REST_Controller {
...
$stmt_item = $this->db->query("select quantity as NQID from adv_pos_item where  merchant_id='".$merchant_id."' AND id  ='".$_POST['item_id']."' AND status=0 ");
			...
			$stmt_cart = $this->db->query("select quantity as NQCID from adv_pos_cart_item where  merchant_id='".$merchant_id."' AND deviceToken='".$deviceToken."' AND item_id  ='".$_POST['item_id']."' AND status=0 ");
		...
		try {
		    //echo $_POST['item_id']; die();
				...
				$stmt = $this->db->query("select id as CID,quantity from adv_pos_cart_item where  merchant_id='".$merchantID."' AND user_id='".$_POST['user_id']."' AND deviceToken='".$deviceToken."' AND item_id  ='".$itemID."' AND status=0 ");
				$package_data = $stmt->result_array();
...
public function getCartItem_post()
{
    ...
    if($merchant_id == $data->merchant_id)
    {
      $userdata = array();
			...
			ct.quantity*it.price as itemCost,discount, CONCAT('".$path."', it.item_image) as itemImage FROM `adv_pos_cart_item` ct join adv_pos_item it 
			on it.id=ct.item_id 
			 where ct.merchant_id='".$_POST['merchant_id']."' and ct.deviceToken='".$_POST['deviceToken']."' and ct.user_id='".$_POST['user_id']."' and ct.status=0 order by ct.id desc");
			...
			foreach ($package_data1 as $each) {
						...
						if ($each['discount'] == "0" || $each['discount'] == null) {
						$package['price'] = (float) round($each['new_price'],2);
						$package['item_price'] = (float) round($each['price'],2);
						} else {
						$package['price'] = (float) round($each['new_price'],2);
						$package['disconted_price'] = (float) round($each['price'],2);
					    }
						$package['tax'] = $each['tax'];
						$package['quantity'] = $each['quantity'];
			...
			//if ($stmt_a->num_rows() > 0) {
 			$getDashboardData = $stmt_a->result_array();
 	            ...
 	            if(isset($type) && strtolower($type) == strtolower("$")){
				    ...
				    $otherchargess =  (float) number_format(($percentage),2);
				    $othercharges = $otherchargess ? $otherchargess : "";
				    $newprice = (float) round(($newprice + $percentage),2);
				}
			else if(isset($type) && strtolower($type) == strtolower("%")){
				    
				   $otherchargess = (float) number_format(($newprice * ($percentage / 100)),2);
				   $othercharges = $otherchargess ? $otherchargess : "";
				    $newprice = (float) round($newprice + ($newprice * ($percentage / 100)),2);
				}
  ...
  public function getFinalTax_post()
{
    ...
    if($merchant_id == $data->merchant_id)
    {
	  ...
	  where ct.merchant_id='".$_POST['merchant_id']."' and ct.deviceToken='".$deviceToken."' and ct.user_id='".$_POST['user_id']."' and ct.status=0 order by ct.id desc");
	  $package_data1 = $stmt1->result_array();
...
// 			print_r($rows);die;
		...
		//	if (isset($package_data)) {
			foreach ($package_data as $each) {
				...
				user_id ='".$_POST['user_id']."' and deviceToken='".$deviceToken."' and status=0");
  ...
  public function clearCartItem_post()
{
    ...
    if($merchant_id == $data->merchant_id)
    {
		...
		$stmt = $this->db->query("DELETE FROM `adv_pos_cart_item`  where  merchant_id='".$_POST['merchant_id']."' and user_id='".$_POST['user_id']."' and deviceToken='".$deviceToken."' and status=0 ");
 ...
 public function deleteCartItem_post()
{
    ...
    if($merchant_id == $data->merchant_id)
    {
		...
		if (isset($_POST['merchant_id'], $_POST['item_id'])) {
 			$stmt = $this->db->query("delete FROM `adv_pos_cart_item`  where id='".$_POST['item_id']."' and merchant_id='".$_POST['merchant_id']."' and deviceToken='".$_POST['deviceToken']."' and status=0 ");
    ...
    if($merchant_id == $data->merchant_id)
    {
		...
		if (isset($_POST['merchant_id'], $_POST['item_id'], $_POST['quantity'], $_POST['price'])) {
			try {
				...
				$new_price = $_POST['price'];
				$price = $_POST['price'] * $quantity;
				$stmt2 = $this->db->query("UPDATE adv_pos_cart_item SET quantity='".$quantity."', price='".$price."',new_price='".$new_price."',deviceToken='".$deviceToken."',discount=0 WHERE merchant_id='".$merchantID."' AND id ='".$itemID."' AND status=0");
		...
		if (isset($_POST['merchant_id'],$_POST['user_id'], $_POST['item_id'], $_POST['quantity'], $_POST['price'])) {
			try {
			    if(isset($_POST['deleted_id']) && $_POST['deleted_id'] !=$_POST['item_id']){
					   ...
					   $stmt = $this->db->query("select count(*) as CID from adv_pos_cart_item where  merchant_id='".$merchantID."' AND  deviceToken='".$deviceToken."' AND user_id='".$_POST['user_id']."' AND item_id  ='".$itemID."' AND status=0 ");
				       $galTotal = $stmt->num_rows();
        				//$row = $stmt->get_result()->fetch_row();
        				// print_r($row);die;
        				//$galTotal = $row[0];
           ...
           // end tax 
        			$stmt3 = $this->db->query("delete FROM `adv_pos_cart_item`  where id='".$_POST['deleted_id']."' and merchant_id='".$_POST['merchant_id']."' and deviceToken='".$deviceToken."' and status=0 ");
				...
				$stmt_newitem = $this->db->query("select item_id as NEID from adv_pos_cart_item where  merchant_id='".$merchantID."' and deviceToken='".$deviceToken."'  AND id  ='".$_POST['item_id']."' AND status=0 ");
			...
			$stmt_cart = $this->db->query("select quantity as NQCID from adv_pos_cart_item where  merchant_id='".$merchantID."' and deviceToken='".$deviceToken."' AND item_id  ='".$new_item_id."' AND status=0 ");
				$package_data_cart = $stmt_cart->result_array();
				$cart_quantity=$package_data_cart[0]['NQCID'];
				$available_quantity=$item_quantity-$cart_quantity;
				$required_quantity = $_POST['quantity'];
			...
			if($cart_quantity==$quantity){
				...
				discount='".$discount."', 
				discount_amount='".$discount_amount."'
				WHERE merchant_id='".$merchantID."' and deviceToken='".$deviceToken."' AND id ='".$itemID."' AND status=0");
				}
  ...
  public function soledCartItem_post()
{
    ...
    if($merchant_id == $data->merchant_id)
    {
		    ...
		    $order_id = $_POST['order_id'] ? $_POST['order_id'] : "";
 			...
 			$stmt2 = $this->db->query("select item_id as itemID, quantity from adv_pos_cart_item  where merchant_id='".$_POST['merchant_id']."' and user_id='".$_POST['user_id']."' and deviceToken='".$deviceToken."' and status=0");
		...
		$items = $mem;
			...
			$stmt = $this->db->query("UPDATE adv_pos_cart_item SET status=2, transaction_id='".$transactionID."',order_id='".$order_id."',tax='".$_POST['tax_status']."'  where merchant_id='".$_POST['merchant_id']."' and deviceToken='".$deviceToken."' and user_id='".$_POST['user_id']."' and status=0");
