...
class Refund_invoice_api extends REST_Controller {
...
public function create_refund_cash_post()
{
...
public function create_refund_cp_post()
{
    // Call the verification method and store the return value in the variable
    $data = array();
    $data = $this->verify_request();
    ...
    if($merchant_id = $data->merchant_id)
    {
			...
			if (isset($_POST['split_payment_id']) && !empty($_POST['split_payment_id'])) {
				...
				$paid_amount = $amount;
  				...
  				$amount = $getEmail[0]['amount'];
	//if($getEmail[0]['amount'] > $p_ref_amount )
   if($amount > $p_ref_amount )
	{
	$p_ref_amount = $p_ref_amount;
	$partial_refund = 1;
	$refund_amount = $p_ref_amount;
	
	$remain_amount = number_format(($amount-$p_ref_amount),2);
	}
	elseif($amount == $p_ref_amount)
	{
       $p_ref_amount = 0;
	   $partial_refund = 0;
...
public function create_refund_cnp_post()
{
    // Call the verification method and store the return value in the variable
    $data = array();
    $data = $this->verify_request();
	...
	$partial_refund = 1;
	$refund_amount = $p_ref_amount;
	$remain_amount = $getEmail[0]['amount'] - $p_ref_amount  ;
	}
	elseif($getEmail[0]['amount'] == $p_ref_amount)
	{
       $p_ref_amount = 0;
	   $partial_refund = 0;
	   $refund_amount = $getEmail[0]['amount'];
	   $remain_amount = 0 ;
	}
