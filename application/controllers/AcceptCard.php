...
class AcceptCard extends CI_Controller {
	...
	public function updateCardByMerchant($invoice_no,$row_Id) {
		...
		if (!empty($security_key) and !empty($processor_id) ) {
			...
			if($response['responsetext']!='Approved'){
				$msg='Cant update card due to '.$response['responsetext'];
				//echo $msg;die;
			...
			$this->acceptcard_model->insert_Card($mydata); 
			  $token=$mydata['token'];
			  $this->acceptcard_model->updateInvoiceAndToken($token,$merchant_id,$invoice_no,$response,$row_Id);
	  			$res=$this->acceptcard_model->fetch($token,$merchant_id);
        ...
        if (!empty($security_key) and !empty($processor_id)) {
            ...
            if ($data = curl_exec($ch)) {
	          ...
	          // }
	            ...
	            if($token_id != '') {
		            ...
		            $res = $this->acceptcard_model->fetch($mydata['token'],$merchant_id);
          ...
          // die("ok");    
	                    ...
	                    $data['msgData'] = $data;
             			...
             			$new_data = array();
	                    $sql_nn="SELECT id,card_type, right(card_no,4) as 'card_no',token from token where token='".$mydata['token']."'";
