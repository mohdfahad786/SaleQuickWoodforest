<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
      ...
      class Employee extends CI_Controller
      {
       public function __construct()
        ...
        public function index(){
            ...
            $merchant_id11 = $this->session->userdata('merchant_user_type');
  ...
  $mem1 = array();
    ...
    $data['item2'] = $mem2;
          ...
          {
           return $this->load->view('employee/dashboard',$data);
          }
     ...
     $fee_email =$merchantdetails['0']['text_email'];
      ...
      $Activate_Details = $this->session->userdata('Activate_Details');
          ...
          $data['action'] = "Send Request";
                  ...
                  $reference = $this->input->post('reverence') ? $this->input->post('reverence') : "";
              ...
              $fee=$fee+$fee_swap+$fee_email;
                 ...
                 $invoice_no = 'INV'.strtoupper($names).'000'.$inv; 
                          ...
                          $merchant_id = $this->session->userdata('merchant_id');
                           $p_merchant_id = $this->session->userdata('p_merchant_id');
                 
                  if ($this->form_validation->run() == FALSE) {
                      $this->load->view('merchant/add_straight_request');
                  } else {
            $today1 = date("Ymdhms");
            $url = 'https://salequick.com/payment/PY'.$today1.'/'.$p_merchant_id;
            $today2 = date("Y-m-d");
            $year = date("Y");
            $month = date("m");
            $time11 = date("H");
...
$getDashboardData_t = $getDashboard_t->result_array();
       ...
       $getDashboard_m = $this->db->query(" SELECT business_dba_name,logo FROM merchant WHERE id = '".$p_merchant_id."'  "); 
...
$message = mail($email_id, $MailSubject, $htmlContent, $header);
              ...
              $this->session->set_userdata("mymsg",  "New payment Request Add Successfully.");
                     // redirect("merchant/all_straight_request");
                      ...
                      redirect(base_url().'Pos_employee/all_customer_request');
                      
                  }
              } 
           else {
                  $this->load->view("employee/add_straight_request" , $data);
              }
  ...
  elseif($merchant_status=="Waiting_For_Approval")
      ...
      $item = array();
          ...
          $merchant_id = $this->session->userdata('merchant_id');
                ...
                //$data['pay_report'] = $this->admin_model->search_pos($searchby);
                 echo $this->load->view('employee/dashboard', $data);
