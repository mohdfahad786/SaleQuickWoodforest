			...
			if(count($getallRecurringRecord)) {
				foreach ($getallRecurringRecord as $key => $row) {
					...
					if( $d==$g && $a =='start'  &&  $row['status']!='confirm' && $row['status']!='Chargeback_Confirm'  ) {           
						...
						$getQuery_tt = $this->db->query(" SELECT invoice_no,token_id FROM invoice_token WHERE invoice_no='$invoice_no'   AND merchant_id='$merchant_id' and status='1' and payroc='1' ");
