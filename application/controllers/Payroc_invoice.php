<?php 
    ...
    class Payroc_invoice extends CI_Controller {
        ...
        public function Stop_recurring($invoice_no,$merchant_id) {
          ...
          public function card_payment() {
            ...
            if ($id && $bct_id1 && $bct_id2) {
                ...
                $this->load->view('payment_card_payment', $data);
                ...
                if ($array['Response']['ExpressResponseMessage'] = 'ONLINE') {
                    if ($paymentcard == 'newcard') {
                       ...
                       $query  = "";
                        ...
                        // $query .= "ipaddress=" . urlencode($ipaddress) . "&";
                        // $query .= "orderid=" . urlencode($orderid) . "&";
                        // if($paytype==1){ $query .= "customer_vault=add_customer". "&";}
                        // $query .= "type=sale";
...
//                       echo "<br/>";
                ...
                //     $request_detail = $this->Admin_model->insert_data("request", $branch_info);
...
//print_r($getEmail); die();
                            ...
                            //$merchnat_msg = $this->load->view('email/merchant_receipt_dash', $data, true);
                             $merchnat_msg = $this->load->view('email/merchant_new_receipt_dash', $data, true);
...
// push notification
                           // if($bct_id2=='413' && $getEmail_a[0]['push_notify']=='1')
                                 if( $getEmail_a[0]['push_notify']=='1')
                              {
...
$getQueryToken = $this->db->query("SELECT * from notification_token where merchant_id ='" . $bct_id2 . "' ");
...
$requestData=array("title"=>'Invoice Payment',"body"=>'Invoice Payment received amount :$'.$getEmail[0]['amount']);

$ntobj=array("title"=>'Invoice Payment',"body"=>'Invoice Payment received amount :$'.$getEmail[0]['amount']);
foreach ($getToken_detail as $each) {
//$firebase_token="dX5_TVr2bUwLhHVxWP-SQU:APA91bGq_FzzJyGjOU0BusmE8M_vyAK09-ElAZY-l0UNjP-pEF5nN3QSxfIArfcKZtmQtEalnbuqRR_MSVODKRW-KeYH81C7Bmx_Z0FlcvL_WsExFnkw6oJXMSjNTkdZERAV3R8mfBQp";
...
if(!empty($getEmail_a[0]['mob_no']) &&  $getEmail_a[0]['sms_notify']=='1') {
    $sms_reciever = $getEmail_a[0]['mob_no'];
   // $sms_message = trim('Payment Receipt : '.$purl);
    $sms_message = trim('Invoice Payment received amount :$'.$getEmail[0]['amount']);
