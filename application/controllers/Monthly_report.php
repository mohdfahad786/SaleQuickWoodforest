...
$refund_data_online = $this->Inventory_graph_model->get_full_refund_online($start_date, $end_date,'pos',$merchant_id);	
  ...
  <br >&nbsp;&nbsp;<span style="font-size: 15px;font-weight:800;">Transactions across all terminals  </span> 
	...
	public function rm() {
		...
		$get_merchant_data = $this->Email_report_model->get_merchant_data_new_monthly_report();
			...
			foreach ($get_merchant_data as $key => $value) {
				...
				if( $Currentdate == $Starting_date  ) //   $Currentdate == $Starting_date
				{
					...
					$msg = $this->load->view('email/reporting', $reporting_data, true);
						...
						$id11 = $this->admin_model->insert_data("monthly_report_email_1", $branch_info);
						...
						$id111 = $this->admin_model->insert_data("monthly_report_email_2", $branch_info1);
	
$getQuery1 = $this->db->query("SELECT count(id) as id from monthly_report_email where merchant_id ='" . $merchant_id . "' and date ='" . $last_date . "'  ");
         ...
         if ($id==0) { 
					...
					//if (!empty($email) && empty($report_email)) {  
						...
						$this->email->subject('Salequick Monthly Reporting');
						$this->email->message($msg);

						$getQuery_mail_count = $this->db->query("SELECT count(id) as id from monthly_report_email where merchant_id ='" . $merchant_id . "' and date ='" . $last_date . "'  ");
         ...
         if ($id_count==0) { 
						...
						$id11 = $this->admin_model->insert_data("monthly_report_email", $branch_info);
		...
		public function email_report_monthly_single() {
		$data = array();
		// echo "this is the report";die;
		//$get_merchant_data = $this->admin_model->get_merchant_data_new_admin();
		$get_merchant_data = $this->Email_report_model->get_merchant_data_new_monthly_report_single();
		...
		if (!empty($get_merchant_data)) { 
			foreach ($get_merchant_data as $key => $value) {
				...
				if( $Currentdate == $Starting_date  ) //   $Currentdate == $Starting_date
				{
					...
					$getQuery_pdf = $this->db->query("SELECT pdf from monthly_report_pdf where merchant_id ='" . $merchant_id . "'   ");
						$getpdf = $getQuery_pdf->result_array();
				
					 $pdf_link =$getpdf[0]['pdf'];
					...
					$reporting_data['address1'] = $value->address1;
					$reporting_data['report_type'] = "Monthly";
					//$reporting_data['pdf_link'] = base_url().'monthly_report/' . $pdf_link;
					// echo "<pre>";print_r($reporting_data);die;
					// die;
					// $data['msgData'] = $data;
					$msg = $this->load->view('email/reporting', $reporting_data, true);
...
$getQuery1 = $this->db->query("SELECT count(id) as id from monthly_report_email where merchant_id ='" . $merchant_id . "' and date ='" . $last_date . "'  ");
		...
		public function email_report_monthly_user() {
		$data = array();
		// echo "this is the report";die;
		//$get_merchant_data = $this->admin_model->get_merchant_data_new_admin();
		$get_merchant_data = $this->Email_report_model->get_merchant_data_new_monthly_user();
		//echo "<pre>";print_r($get_merchant_data);die;
		if (!empty($get_merchant_data)) { 
			foreach ($get_merchant_data as $key => $value) {
				// $merchant_id = $this->session->userdata('merchant_id');
...
public function email_report_monthly_user_single() {
		$data = array();
		// echo "this is the report";die;
		//$get_merchant_data = $this->admin_model->get_merchant_data_new_admin();
		$get_merchant_data = $this->Email_report_model->get_merchant_data_new_monthly_user_single();
		//echo "<pre>";print_r($get_merchant_data);die;
		if (!empty($get_merchant_data)) { 
			foreach ($get_merchant_data as $key => $value) {
				// $merchant_id = $this->session->userdata('merchant_id');
