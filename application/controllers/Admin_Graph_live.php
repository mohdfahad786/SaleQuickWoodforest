...
class Admin_Graph extends CI_Controller {
	...
	public function getSalesSummaryChartData() {
		...
		$tableData = array();
		if ($employee == 'all') {
			$stmt = $this->db->query("SELECT (SELECT SUM(amount) as Amount from customer_payment_request where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as Amount,
				(SELECT SUM(amount) as PAmount from pos where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as PAmount,
				(SELECT SUM(tax) as Tax from customer_payment_request where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as Tax,
				(SELECT SUM(tax) as PTax from pos where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as PTax,
				(SELECT SUM(other_charges) as c_other_charges from customer_payment_request where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as c_other_charges,
				(SELECT SUM(other_charges) as p_other_charges from pos where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as p_other_charges,
				(SELECT SUM(tip_amount) as tip from customer_payment_request where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as Tip,
				(SELECT SUM(tip_amount) as PTip from pos where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as PTip,
				...
				(SELECT SUM(fee) as Fee from customer_payment_request where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as Fee,
				(SELECT SUM(fee) as PFee from pos where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and (status='confirm' or status='Chargeback_Confirm')) as PFee,
				(select sum(r.amount) as rAmu from pos p join refund r on p.invoice_no=r.invoice_no where r.date_c>='".$date_c."' and r.date_c <= '".$date_cc."' and  p.status='Chargeback_Confirm') As RAmountPOS,
  				(select sum(r.amount) as rAmu from customer_payment_request p join refund r on p.invoice_no=r.invoice_no where r.date_c>='".$date_c."' and r.date_c <= '".$date_cc."' and  p.status='Chargeback_Confirm' ) As RAmountCPR");
			...
			$stmt = $this->db->query("SELECT (SELECT SUM(amount) as Amount from customer_payment_request where date_c >= '".$date_c."' and date_c <= '".$date_cc."' and merchant_id= $employee and (status='confirm' or status='Chargeback_Confirm') ) as Amount,
				...
				(select sum(r.amount) as rAmu from pos p join refund r on p.invoice_no=r.invoice_no where r.date_c>='".$date_c."' and r.date_c <= '".$date_cc."' and r.merchant_id= $employee and  p.status='Chargeback_Confirm') As RAmountPOS,
  				(select sum(r.amount) as rAmu from customer_payment_request p join refund r on p.invoice_no=r.invoice_no where r.date_c>='".$date_c."' and r.date_c <= '".$date_cc."' and r.merchant_id= $employee and p.status='Chargeback_Confirm') As RAmountCPR");
		}
		// echo $this->db->last_query();die;
		...
		if ($stmt->num_rows() > 0) {
			...
			foreach ($stmt->result_array() as $result2) {
				...
				$temp5 = array(
					...
					'clicks' => '0',
					'converted_people' => $result2['c_other_charges'] + $result2['p_other_charges'],
				);
