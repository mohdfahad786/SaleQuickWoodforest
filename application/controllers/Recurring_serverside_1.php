    ...
    public function index() {
        ...
        foreach ($recur_list as $val) {
            ...
            if($val->recurring_count_remain =='0') {
                $action_btn = '<a class="pos_vw_refund pos_Status_c badge-btn" href="'.base_url().'pos/invoice_details/'.$val->payment_id.'"><span class="fa fa-eye"></span> Transactions</a>';
            } else {
                $edit_link = base_url().'editRecurring/edit_customer_request/'.$val->id;
                $refund_link = base_url().'pos/invoice_details/'.$val->payment_id;
