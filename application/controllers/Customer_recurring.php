        ...
        if (isset($_POST['submit'])) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($paytype == '0' || $paytype == '1') {
                    ...
                    $token_id_new = $_POST['new_token_input'];

                    if( $paytype=='1'  && $token_id_new!='' && $today2 == $recurring_pay_start_date) {
                        $getMerchantdata = $this->db->query("SELECT * from merchant where id ='".$merchant_id."' ");
                        $get_token_value = $this->db->where('id', $token_id_new)->get('token')->row();
                        // echo $get_token_value->token;die;
                        $mydata['token'] = $get_token_value->token;
                    ...
                    $token_value = !empty($token_id_new) ? '1' : '0';
                    ...
                    if(!empty($token_id_new)) {
                        $ss_token_id = $token_id_new;
                    } else {
                        $ss_token_id = '0';
                    }

                    $data_inv_token = array(
                        'token_id' => $ss_token_id,
                        'invoice_no' => $invoice_no,
                        'merchant_id' => $merchant_id,
                    );
                    ...
                    //print_r($collect);die;
                    // $this->acceptcard_model->saveInvoiceNum($collect, $token_id_new);
