...
// 			`cart`.`merchant_id` = '$merchant_id' AND `item`.`merchant_id` = '$merchant_id' AND `item`.`item_id`='$main_items' AND `item`.`mode`='1'  
// 			GROUP BY `main_item_id` ORDER BY `item`.`item_id` DESC"); 
			
			
			 $query=$this->db->query("SELECT `cart`.`item_id` as `item_id` ,`item`.`item_id` as `main_item_id`,`posmain`.`name` as `mname`,item.sku, `item`.`name` as `item_name`, `item`.`title` as
            `item_title`, SUM(`item`.`price`) as `base_price`,SUM(`cart`.`quantity`) as `quantity`,SUM(round((`cart`.`price`)*(`item`.`tax`)/100,2)) as `tax_value`, 
	...
	public function get_full_inventory_reportdata_main_no_main_item1($start_date, $end_date,$merchant_id) { 
		...
		SUM(`item`.`price`) as `base_price`,SUM(`cart`.`quantity`) as `quantity`,SUM(round((`cart`.`price`)*(`item`.`tax`)/100,2)) as `tax_value`,sum(`cart`.`discount_amount`)  as `discount`, 
