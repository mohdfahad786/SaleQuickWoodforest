...
class Acceptcard_model extends CI_Model {
	public function __construct() {
      ...
      public function updateInvoiceAndToken($token,$merchant_id,$invoice_no,$response){
        ...
        if($response['responsetext']!='Approved'){
           $q1="update customer_payment_request set status='declined',message='".$response['responsetext']."',auth_code='".$response['authcode']."',transaction_id=".$response['transactionid'].",card_no='".$response['cc_number']."',card_type='".$response['cc_type']."'  where invoice_no='".$invoice_no."'";
            $this->db->query($q1);
          // echo $this->db->last_query();die; 
        }
        else{
            $r_count=$this->db->query("SELECT recurring_count from customer_payment_request where invoice_no='".$invoice_no."'")->result_array();
        
            $recurringcount= $r_count[0]['recurring_count'];
        
            if($recurringcount!=-1){
                $q1="update customer_payment_request set token=1,status='confirm',message='".$response['responsetext']."',auth_code='".$response['authcode']."',transaction_id=".$response['transactionid'].",card_no='".$response['cc_number']."',card_type='".$response['cc_type']."', recurring_count_paid=recurring_count_paid+1,recurring_count_remain=recurring_count_remain-1 where invoice_no='".$invoice_no."'";
            $this->db->query($q1);

            }
            else{
                $q1="update customer_payment_request set token=1,status='confirm',message='".$response['responsetext']."',auth_code='".$response['authcode']."',transaction_id=".$response['transactionid'].",card_no='".$response['cc_number']."',card_type='".$response['cc_type']."', recurring_count_paid=recurring_count_paid+1,recurring_count_remain=1 where invoice_no='".$invoice_no."'";
                $this->db->query($q1);
          }
        // echo $this->db->last_query();die; 
        }
      ...
      public function updateRecurringDeclined($invoice_no,$response)
      {
        $q1="update customer_payment_request set status='declined',message='".$response['responsetext']."',auth_code='".$response['authcode']."',transaction_id=".$response['transactionid'].",card_no='".$response['cc_number']."',card_type='".$response['cc_type']."'  where invoice_no='".$invoice_no."'";
          $this->db->query($q1);
          // echo $this->db->last_query();die; 
      }
      public function updateRecurringCount($invoice_no,$response)
      {
        $r_count=$this->db->query("SELECT recurring_count from customer_payment_request where invoice_no='".$invoice_no."'")->result_array();
        ...
        if($recurringcount!=-1){
          ...
          $q1="update customer_payment_request set status='confirm',message='".$response['responsetext']."',auth_code='".$response['authcode']."',transaction_id=".$response['transactionid'].",card_no='".$response['cc_number']."',card_type='".$response['cc_type']."', recurring_count_paid=recurring_count_paid+1,recurring_count_remain=1 where invoice_no='".$invoice_no."'";
          $this->db->query($q1);
        }
        // echo $this->db->last_query();die; 
      }
