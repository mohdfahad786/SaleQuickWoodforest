...
// echo $this->db->last_query(); die("");   
	...
	public function get_full_inventory_reportdata_main_no_main_item($start_date, $end_date,$merchant_id,$main_items) { 
		...
		`item`.`title` as `item_title`,`cart`.`bill_tax` as `bill_tax`,`cart`.`bill_discount` as `bill_discount`,SUM(round(tax_value,2)) as `tax_value`,  round(`item`.`tax`,2) as `tax_value1` ,item.sku,SUM(`cart`.`quantity`) as `quantity`,
