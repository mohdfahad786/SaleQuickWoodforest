...
public function inactive_merchant() {
	...
	public function report() {
		...
		// foreach ($package_data as $each) {
		  ...
		  $package['name'] =$total_payout[0]['business_dba_name'];
            ...
            //die();
             ...
             $package['m_cost'] = $package_data_report_merchant[0]['interchangeFee']+$package['m_buy_rate']+$package['m_gateway_fee'];

             $package['m_cost_only'] = $package_data_report_merchant[0]['interchangeFee'];

             $profit = $package_data_report_merchant[0]['GrossProfit'] -($package['m_buy_rate']+$package['m_gateway_fee']);
		...
		$data['Cost'] = $package_data_full_reporst[0]['interchangeFee']+$package_data_full_reporst[0]['buy_rate']+$package_data_full_reporst[0]['buy_rate_valume']+$package_data_full_reporst[0]['gateway_fee'];
		$data['meta'] = 'Residual';
			...
			// print_r($package); die; 
     		foreach($package as $pak) {
          		...
          		$data['commission_p_merchant'] = $pak->commission_p_merchant;
           		$data['commission_p_transaction'] = $pak->commission_p_transaction;
          		$data['buy_rent'] = $pak->buy_rent;
          		$data['buyrate_volume'] = $pak->buyrate_volume;
          		break;
     		}
		}
 		$data['loc'] = "edit_profile";
		...
		if ($password_one == $cpassword) {
			...
			if(count($usr_result) =='0') {
				...
				if ($email && $password_one && $cpassword) {
					...
					$upAssgnMerchant = $assign_merchant.','.$last_merchantId;
					$upData = array('assign_merchant' => $upAssgnMerchant);
					$this->db->where('id', $agent_id);
					$this->db->update('sub_admin', $upData);
					//echo $this->db->last_query();die();
