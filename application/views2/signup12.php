              ...
              <div class="third-step row <?php if($this->session->userdata('step')=="two"){ echo 'active'; } ?>" data-fStep="3">
                ...
                <div class="busi_owner_inputs">
                    <div class="first_busi_owner_section pointer">
                        ...
                        <div class="col-12">
                          <div class="form-group">   
                            <input type="text" class="form-control us-phone-no cl_fo_phone" value="<?php if($Merchant) echo $Merchant['o_phone']; ?>" name="fo_phone" placeholder="Phone Number" required autocomplete="off" >
                          </div>
                        </div>
                        <div class="col-12">
                          <div class="form-group">   
                            <input type="text" class="form-control email cl_fo_email" value="<?php if($Merchant) echo $Merchant['o_email']; ?>" name="fo_email" placeholder="Email Address" required autocomplete="off">
                          </div>
                        </div>
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        else if(data=='600')
        {
          ...
          $('.alertmessage').html('<div class="alert alert-danger text-center">both password Are not Match!</div>');
        }
        else if(data=='700')
        {
            $('.custom-stepper-form .first-step .stepper-submit span').removeClass('fa fa-spinner fa-spin');
      ...
      success: function(data) {
               ...
               if(data.Status=='200')
        {
          window.location.href='<?=base_url("login");?>';
           ...
           $('.alertmessage').html('<div class="alert alert-danger text-center">Send Successfully!2</div>');
        }
         if(data.Status=='30')
        {
          window.location.href='<?=base_url("login");?>';
...
$('.alertmessage').append('<span class="text-danger">'+data.Errors[i].Message+'</span><br/>');
    ...
    $(document).on('click', '.add_more_owner', function() {
        if($('.first_busi_owner_section').hasClass('pointer')) {
            alert('Add Business Owner Info first.');
            return false;
