...
currency_symbol = '$';
     ...
     $data = $this->admin_model->data_get_where_1('merchant', array('status' => 'active', 'user_type' => 'merchant')); ?>
                       <select name="employee" class="form-control bder-radius" id="employee" onchange="getGraph();getTable();onSelectChange3()" >
...
function onSelectChange3(start, end){
    ...
    type:'post',
    beforeSend: function() {
        $('#chart1').addClass('custom_loading');
    },
    success: function (dataJson)
        {
            fullData = JSON.parse(dataJson);
            data = fullData.getDashboardData[0];
            // data = JSON.parse(dataJson).getDashboardData[0];
            console.log(data)
...
//console.log(data)
    ...
    $('#chart1').removeClass('custom_loading');
};
