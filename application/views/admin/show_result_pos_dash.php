...
foreach ($pay_report as $row) { ?>
    ...
    <div class="modal-body">
        <div id="new_receipt_body" class="col-12">
            ...
            <div class="row <?php echo $row->c_type; ?>" style="margin-bottom: 20px !important;">
                <div class="col-sm-6 col-md-6 col-lg-6"> 
                    <div class="display-avatar" style="padding: 0px !important;">
                        <?php if(!empty($merchant_details[0]['logo'])) { ?>
                            <img class="invoice-logo" src="<?php echo base_url()."logo/".$merchant_details[0]['logo']; ?>" alt="logo">
                        <?php } else { ?>
                            <div class="img-lg-custom-text">
                                <?php echo !empty($merchant_details[0]['business_dba_name']) ? substr($merchant_details[0]['business_dba_name'],0,1) : ''; ?>
                            </div>
                        <?php } ?>
                    </div>
                </div>
                    ...
                    <table class="payment-details-table" style="width: 100%">
                        <tbody>
                            <?php if($row->transaction_type == 'split') { ?>
                                ...
                                <?php if(isset($row->discount) && $row->discount!='0' && $row->discount!='' && $row->discount!='$0.00') {
                                    $DisAmount = $row->tip_amount+$row->tax + $row->total_amount - $row->full_amount;
                                    
                                    if($DisAmount!=0) { ?>
                                        <tr>
                                            <td width="50%" class="left" style="color: rgb(251, 32, 10);">Total Discount</td>
                                            <td width="50%" style="text-align: right;color: rgb(251, 32, 10);">$ <?php echo number_format($DisAmount,2);  ?></td>
                                        </tr>
                                    <?php }
                                } ?>
                                ...
                                <!--    <td width="50%" style="text-align: right;">$ <?php echo number_format($row->amount-($row->tax+$row->tip_amount+$row->other_charges),2); ?></td>-->
                                <!--</tr>-->
                                
                                <?php if($merchant_details[0]['tip'] > 0) { ?>
                                    <tr>
                                ...
                                <?php  if(isset($item) && count($item) > 0  && $row->tax  > 0) { ?>
                                    <tr>
                                ...
                                <?php if(isset($row->other_charges) && !empty($row->other_charges) && $row->other_charges!=0) {?>
                                    <tr>
                                        <td width="50%" class="left"><?php echo  $row->otherChargesName; ?></td>
                                        <td width="50%" style="text-align: right;"><?php echo (isset($row->other_charges) && !empty($row->other_charges))? '$ '.number_format($row->other_charges,2) : "-"; ?></td>
                                    </tr>
                                <?php } ?>
                                
                                <tr>
                                    <td width="50%" class="left" style="font-size: 18px;">Total Amount</td>
                                    <td width="50%" style="text-align: right;font-size: 18px;"><?php echo (isset($row->full_amount) && !empty($row->full_amount)) ? '$ '. number_format($row->full_amount,2) : "-"; ?></td>
                                </tr>
                                <tr>
                            ...
                            <?php } else { ?>
                                ...
                                <?php if(isset($row->discount) && $row->discount!='0' && $row->discount!='' && $row->discount!='$0.00') {
                                    $DisAmount=$row->tip_amount+$row->tax+$row->total_amount-$row->amount;
                                ...
                                <tr>
                                    <td width="50%" class="left">Sub Amount</td>
                                    <td width="50%" style="text-align: right;">$ <?php echo number_format(floatval($row->amount)-(floatval($row->tax)+floatval($row->tip_amount)+floatval($row->other_charges)),2); ?></td>
                                </tr>
                               ...
                               <?php if(isset($row->tax) && !empty($row->tax) && $row->tax!=0) { ?>
                                    <tr>
                                        <td width="50%" class="left">Total Taxes</td>
                                        <td width="50%" style="text-align: right;">$ <?php echo number_format($row->tax,2); ?></td>
                                    </tr>
                                ...
                                <tr>
                                    <td width="50%" class="left" style="font-size: 18px;">Total Amount</td>
                                    <td width="50%" style="text-align: right;font-size: 18px;">$ <?php echo (isset($row->amount) && !empty($row->amount) )? number_format($row->amount,2) : number_format($row->amount,2); ?></td>
                                </tr>
                                <tr>
                                    <td width="50%" class="left">Transaction ID</td>
                            ...
                            <?php if(!empty($row->reference) && $row->reference!='N/A'){ ?>
                                <tr>
            ...
            <?php if(( $row->status =='confirm'  ||  $row->status =='Chargeback_Confirm' )  && $row->amount > $ttlrefunded   && ( $row->c_type=='CNP' ||  $row->c_type=='CP' ||  $row->c_type=='PAX' ) ) { ?>
                    ...
                    <div class="col-sm-6 col-md-6 col-lg-6">
                        <div class="row">
                            <div class="col-9" style="padding-right: 0px !important;">
                                <input class="form-control fullRefund__amount" readonly name="amount" value="<?php echo $ttlrefunded ? number_format($row->amount-$ttlrefunded,2) :$row->amount; ?>" style="height: 33px !important;border-top-right-radius: 0px !important;border-bottom-right-radius: 0px !important; "  type="text">

                                <input class="form-control partRefund__amount"  name="amount" value="<?php echo $ttlrefunded ? number_format($row->amount-$ttlrefunded,2) :$row->amount; ?>" style="height: 33px !important;border-top-right-radius: 0px !important;border-bottom-right-radius: 0px !important; " data-max="<?php echo $ttlrefunded ? $row->amount-$ttlrefunded :$row->amount; ?>"  type="text" onKeyPress="return isNumberKeyOnedc(this,event)" placeholder="Partial amount">
                            </div>
                            <div class="col-3" style="padding-left: 0px !important;">


                                    <form action="<?=base_url();?><?php if($row->c_type=='CNP' && $row->processor_name=='PAYROC') { echo 'payroc/refund_payroc'; } else if($row->c_type=='CNP'){ echo 'pos/refund_pos'; }else if($row->c_type=='PAX'){ echo 'pos/refund_pos'; }else if($row->c_type=='CP'){ echo 'pos/refund_cp_pos'; }else{ echo "pos/all_pos";} ?>"  abc="<?php echo $row->c_type; ?>" method="post">
                                        ...
                                        <?php  if($row->c_type=='CNP' || $row->c_type=='CP' || $row->c_type=='PAX' ){  ?>
                                            <button type="button" disabled id="receiptSSendRequest" name="submit" class="btn btn-second" style="height: 33px !important;padding-top: 5px !important;border-top-left-radius: 0px !important;border-bottom-left-radius: 0px !important;">Send</button>
                                        <?php } ?>
                                </form>
                            </div>
                        </div>
    ...
    <div class="modal-footer">
        <?php if( ($row->status =='confirm' ||  $row->status =='Chargeback_Confirm' )  && $row->amount > $ttlrefunded ){ ?>
            <div class="col-sm-12 col-md-12 col-lg-12" style="display: inherit !important;">
                <!-- <div class="row"> -->
                    <!-- <div class="col-6"> -->
                        <button type="button" id="receiptSSendRequestPdf" name="submit" class="btn btn-rounded social-btn-outlined" style="font-size: 12px !important;"><i class="mdi mdi-arrow-down medium"></i>Save as PDF</button>

                        <button type="button" id="receiptSSendRequestPrint" name="submit" class="btn btn-rounded social-btn-outlined" style="font-size: 12px !important;margin-left:5px;"><i class="mdi mdi-arrow-down medium"></i>Print Receipt</button>
                    <!-- </div> -->
                    <!-- <div class="col-6"> -->
            ...
            <div class="col-sm-12 col-md-12 col-lg-12">
                <div class="row">
                    <div class="col-sm-6 col-md-6 col-lg-6" style="display: inherit !important;">
                        <button type="button" id="receiptSSendRequestPdf" name="submit" class="btn btn-rounded social-btn-outlined"><i class="mdi mdi-arrow-down medium"></i>Save as PDF</button>

                        <button type="button" id="receiptSSendRequestPrint" name="submit" class="btn btn-rounded social-btn-outlined"><i class="mdi mdi-arrow-down medium"></i>Print Receipt</button>
                    </div>
                    <div class="col-sm-6 col-md-6 col-lg-6">
