<div class="modal-header"> 
	...
	<div class="irm-header-inner">
		<div class="row">
			<div class="col">
				<div class="irm-logo">
					<img src="<?= base_url()."logo/".$merchant_data[0]['logo']; ?>" alt="logo">
				</div>
				<div class="irm-def">
					<h4><?= $merchant_data[0]['business_dba_name'] ? $merchant_data[0]['business_dba_name'] : "-"; ?></h4>
					<p><b>Telephone: </b> <?= $merchant_data[0]['business_number'] ? $merchant_data[0]['business_number'] : "-"; ?></p>
				</div>
			</div>
			<div class="col">
				<div class="irm-info">
			...
			<div class="irm-pay-detail">
				...
				<div class="irm-pay-detail">
					<?php 
						...
						// if(($invoice_detail_receipt[0]->late_fee) || ($merchant_data[0]->late_fee_status > 0 && date('Y-m-d') > ($invoice_detail_receipt[0]->recurring_pay_start_date , strtotime(" +".$late_grace_period."days")))) { 
						if(!empty($invoice_detail_receipt[0]->late_fee)) { 
					?>
						<p><span>Late Fee :</span> <?php echo (isset($invoice_detail_receipt[0]->late_fee) && !empty($invoice_detail_receipt[0]->late_fee)) ? '$'. $invoice_detail_receipt[0]->late_fee : "0"; ?></p>
					<?php } ?>
		...
		?>
			<form id="sRefform" action="<?php echo base_url();  ?>dashboard/refund" method="post"  style=" display: inline; ">
				<input class="form-control" name="invoice_no" id="invoice_no" 
					...
					<input class="form-control" name="merchant_id" id="merchant_id" 
					value="<?php echo $invoice_detail_receipt[0]->merchant_id ?>"  readonly required type="hidden">
...
<script>
	...
	function resendreceipt(el,rowid) {
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('dashboard/re_receipt'); ?>', 
