...
<!-- Start Page Content -->
    ...
    <div class="col-sm-2">
      <form class="row " method="post" action="<?php echo base_url('pos/all_pos'); ?>" >
              <div class="col-2">
			   <input  name="start_date" type="hidden" value="<?php echo (isset($start_date) && !empty($start_date)) ? $start_date : date('Y-m-d', strtotime('-30 days')); ?>">
                    <input  name="end_date" type="hidden" value="<?php echo (isset($end_date) && !empty($end_date)) ? $end_date : date('Y-m-d'); ?>">
					 <input  name="status" type="hidden" value="<?php echo (isset($status) && !empty($status)) ? $status : ''; ?>">
                <!--<button class="btn btn-first" type="submit" name="search_Submit" value="Search">Export Pdf</button>-->
              </div>
            </form>
    </div>
	</div>
