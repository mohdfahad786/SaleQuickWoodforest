					...
					<div class="col">
						...
						<div class="recipt" style="clear: both;width: 100%;">
									 ...
									 if($pos['discount_amount'] > 0){ 
										...
										<span style="color: #8e9292;font-size: 10px;">'.$pos_title.' </span> </td><td style="padding: 5px 1px;">'.$pos['quantity'].'</td><td style="padding: 5px 1px;"> ( <del style="color:red">$'.number_format(($pos['price']+$pos['discount_amount']),2).'</del> )   $'.number_format($pos['price'],2).'</td></tr>';
									 }
									 else
									 {
									     	$recipt=$recipt.'<tr><td style="padding: 5px 1px;"></td><td style="padding: 5px 1px;">'.$pos['name'].' <br>
							...
							{   ?>
								...
								<p><span>Tip Amount : </span> $<b ><?php  echo number_format($row->tip_amount,2); ?></b></p>
								...
								{  ?>
									...
									<p><span><?php echo  $row->otherChargesName ?> :</span> $<b ><?php echo (isset($row->other_charges) && !empty($row->other_charges))? $row->other_charges : "-"; ?></b></p>
									<?php } ?>
							...
							<?php }?>
								...
								<?php if($row->reference_numb_opay) { ?>
									<p><span>OPay Reference Number :</span> <?php echo (isset($row->reference_numb_opay) && !empty($row->reference_numb_opay))? $row->reference_numb_opay : "-"; ?></p>
								<?php } ?>
							...
							<div class="allpos__reftype">
								...
								<div class="irm-pay-detail custom-form">
									<p>
									 ...
									 <span> <input class="form-control fullRefund__amount" readonly name="amount" value="<?php echo $ttlrefunded ? number_format($row->amount-$ttlrefunded,2) :$row->amount; ?>" style="height: auto !important; "  type="text"></span>
									</p>
    ...
    font-family: cursive;">
												    ...
												    Partial Refund is currently down for maintenance
									...
									<p>
										...
										<span><input class="form-control partRefund__amount"  name="amount" value="<?php echo $ttlrefunded ? number_format($row->amount-$ttlrefunded,2) :$row->amount; ?>" style="height: auto !important; " data-max="<?php echo $ttlrefunded ? $row->amount-$ttlrefunded :$row->amount; ?>"  type="text" onKeyPress="return isNumberKeyOnedc(this,event)" placeholder="Partial amount"></span>
									</p> 
								</div>
							</div>
							<?php }?>
