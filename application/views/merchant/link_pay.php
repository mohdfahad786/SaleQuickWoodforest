...
<script src="<?php echo base_url() ?>new_assets/js/easyrun.qrcode.min.js" type="text/javascript" charset="utf-8"></script>
                            ...
                            <div class="mt-auto">
                                <div class="row">
                                    ...
                                    <div class="col-6 text-right">
                                        <button type="button" class="btn btn-sm btn-primary" id="copyLink" style="height: 30px !important;padding: 6px 10px !important;"><i class="fa fa-files-o"></i></button>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-12">
                                        <input type="text" id="copy_payment_link" disabled class="form-control" value="https://salequick.com/pay_now/<?php echo $merchant_key; ?>/<?php echo $id; ?>">
                                    </div>
                                </div>
                                <hr style="margin: 35px 0px !important;">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="form-title" style="margin-bottom: 10px !important;">Want to show billing address?</div>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-12">
                                        <label class="switch switch_type1" role="switch" style="z-index: 0 !important;">
                                            <input type="checkbox" name="isBilling" id="isBilling" class="switch__toggle" <?php echo ($isBilling == 1) ? 'checked' : ''; ?>>
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <div class="form-title" style="margin-bottom: 10px !important;">Is reference required?</div>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-12">
                                        <label class="switch switch_type1" role="switch" style="z-index: 0 !important;">
                                            <input type="checkbox" name="isReferenceRequired" id="isReferenceRequired" class="switch__toggle" <?php echo ($isReferenceRequired == 1) ? 'checked' : ''; ?>>
                                            <span class="switch__label"></span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div class="row" style="margin-bottom: 15px !important;">
                                    ...
                                    <div class="col-6 text-right">
                                        <button type="button" id="btn-downloadQR" class="btn btn-sm btn-primary" href="" style="height: 30px !important;padding: 7px 10px !important;"><i class="fa fa-download"></i></button>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-12">
                                        <input id="text" type="hidden" value="https://salequick.com/pay_now/<?php echo $merchant_key; ?>/<?php echo $id; ?>" />
                                        <div id="convertToImage" class="convert">
                                            <div style="width: 330px;margin: 10px auto;background-color: #20358e;height: 550px;border-radius: 20px;">
                                                <div style="text-align: center;padding: 60px 20px 0px 20px;">
                                                    <img src="https://salequick.com/front/images/logo-w.png" alt="Salequick" style="width: 200px;margin-bottom: 30px;">
                                                    <div id="container"></div>
                                                    <p style="color: #fff;margin-top: 5px;font-size: 16px;"><?php echo $business_name; ?></p>
                                                    <p style="color: #fff;margin-top: 20px;font-size: 20px;">Scan and Pay</p>
                                                    <p style="color: darkgrey !important;margin-top: 60px;font-size: 11px;">© SaleQuick <?php echo date('Y'); ?> | All rights reserved</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    $(document).on('click', '#isBilling', function() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('merchant/updateLinkPayBillingStatus')?>",
            data:{'isBilling':isBilling},
            success: function(response) {
                //console.log(response);
                if (response == 200) {
                    $('.update_msg').empty();
                    $('.update_msg').html('Updated successfully');
                    $('#message_popup').modal('show');

                } else {
                    $('.update_msg').empty();
                    $('.update_msg').html('Something went wrong. Try again');
                    $('#message_popup').modal('show');
                }
            }
        });
    ...
    $(document).on('click', '#isReferenceRequired', function() {
        // if ($('input.checkbox_check').is(':checked')) {
        if ($('#isReferenceRequired').is(':checked')) {
            console.log(1);
            var isReferenceRequired = 1;
        } else {
            console.log(0);
            var isReferenceRequired = 0;
        }

        $.ajax({
            type: "POST",
            url: "<?php echo base_url('merchant/updateLinkPayReferenceRequired')?>",
            data:{'isReferenceRequired':isReferenceRequired},
            success: function(response) {
                //console.log(response);
                if (response == 200) {
                    $('.update_msg').empty();
...
<script>
    document.getElementById('btn-downloadQR').onclick = function() {
        const screenshotTarget = document.getElementById('convertToImage');
