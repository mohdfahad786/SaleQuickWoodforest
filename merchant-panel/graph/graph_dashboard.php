...
$stmt = $conn->prepare("SELECT id,merchant_id,invoice_no ,sum(amount) as amount,sum(p_ref_amount) as p_ref_amount,sum(tax) as tax,sum(fee) as fee,name,date_c from ( SELECT id,merchant_id,invoice_no,amount,p_ref_amount,tax,fee,name,date_c from customer_payment_request where  date_c >= ? and date_c <= ? and (status='confirm' or partial_refund=1)  union all SELECT id,merchant_id,invoice_no,amount,p_ref_amount,tax,fee,name,date_c from recurring_payment where date_c > ? and date_c < ? and (status='confirm' or partial_refund=1)   union all SELECT id,merchant_id,invoice_no,amount,p_ref_amount,tax,fee,name,date_c from pos where  date_c >= ? and date_c <= ? and (status='confirm' or partial_refund=1) ) x group by date_c");
