...
//invoicing recurring calculation
  function calculateTaxAmount($this){
    ...
    // console.log(ttlTaxWtTax);
    // console.log(ttlTaxWtTax);

    // console.log(taxAmt);
    // console.log(ttlAmtTax);
    $('#sub_amount').val(ttlAmtTax - taxAmt);
  ...
  function calcTaxAmt($this){
      ...
      var totalWtTx =  parseInt(taxAmt) + parseInt(totalAmt);
...
//sale chart default range
  function daterangeDefault(){
    $("#daterange span").data({'d1':moment().subtract(9, 'days').format("YYYY-MM-DD") ,'d2' : moment().format("YYYY-MM-DD")})
...
//draw sales chart
  function salesChart(dataVals1){
    ...
    var allDate=[];
    dataVals1=JSON.parse(dataVals1);
...
// dashboard sale order chart updation fn
  function updateSaleOrderChart(dataVals1,a,b,c){
